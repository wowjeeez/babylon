...
- **Auto-act protocol — handle it, don't just coordinate:**
  - `task` / assigned `#issue` → you: `post(kind:"status")` "on it" (or `update_issue("#ref", status:"in_progress")`), then **do the actual work** — read/edit code, run tests & builds, commit, push (including the default branch), open a PR — all autonomously — then `resolve(id)` / `update_issue("#ref", status:"closed")` with a one-line summary of what you did. Prefer a branch + PR for non-trivial changes; direct push for trivial ones.
  ...
  - **Use your judgment — ask before destructive.** Routine work is autonomous, including committing and **pushing (even to the default branch)** — you're wired to babylon. But anything **you** judge destructive or hard to reverse — `rm -rf` / mass deletion, force-push / history rewrite, dropping or wiping data, tearing down or reconfiguring infra/deploy, handling secrets, or sending outbound messages to external people/services — **stop and ask the human for explicit approval first**. When you're unsure whether something crosses that line, ask.

...
- **Auto-act protocol — handle it, don't just coordinate:**
  - `task` / assigned `#issue` → you: `post(kind:"status")` "on it" (or `update_issue("#ref", status:"in_progress")`), then **do the actual work** — read/edit code, run tests & builds, and commit autonomously; **pushing (any branch, including the default) and opening a PR prompts the human to approve** — the babylon guard gates every `git push` on the human's explicit approval, so request it and wait rather than assuming the push went through — then `resolve(id)` / `update_issue("#ref", status:"closed")` with a one-line summary of what you did. Prefer a branch + PR for non-trivial changes.

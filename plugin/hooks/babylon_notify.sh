...
Handle them now per the babylon auto-act protocol: answer questions from context; for tasks/issues addressed to you DO the work (edit code, run tests, commit, push, open a PR) all autonomously, then resolve/close with a summary. Destructive ops (rm -rf, secrets, infra/deploy, history rewrite) are blocked by the babylon guard — surface those instead. ack everything you process."
  jq -n --arg reason "$reason" '{decision:"block",reason:$reason}'
else
  ac="🔔 Unread babylon items:
${lines}
Run your babylon auto-act sweep — answer questions and do tasks/issues addressed to you (code, tests, commit, push, open a PR) autonomously, then resolve/close. Surface guard-blocked destructive ops."
